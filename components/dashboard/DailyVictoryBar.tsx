...
interface DailyVictoryBarProps {
  percent: number;
}

export function DailyVictoryBar({ percent }: DailyVictoryBarProps) {
  const getColor = () => {
  ...
  return (
    <Card className={cn("p-4 md:p-6 rounded-xl border shadow-lg", getBorderColor())}>
      <div className="mb-3 md:mb-4 flex justify-between items-center">
        <h2 className="text-lg md:text-xl font-bold text-foreground">Daily Victory</h2>
      ...
      <div className="mt-3 flex items-center justify-between text-xs md:text-sm text-muted-foreground">
        ...
        <span className="text-center">
          {percent >= 90 && "🏆 Perfect Day"}
          {percent >= 60 && percent < 90 && "✨ Great Progress"}
          {percent > 0 && percent < 60 && "💪 Keep Going"}
          {percent === 0 && "Start Your Day"}
        </span>
        <span>100%</span>
      </div>
    </Card>
